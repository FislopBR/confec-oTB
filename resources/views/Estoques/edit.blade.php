            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <form action="{{ route('estoques.update', $estoque->id) }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="estoque_id" value="{{ old('estoque_id', $estoque->estoque_id) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    </div>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Capacidade</label>
                            <input type="number" name="capacidade" value="{{ old('capacidade', $estoque->capacidade) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Localização</label>
                            <input type="text" name="localizacao" value="{{ old('localizacao', $estoque->localizacao) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        </div>
                    </div>
