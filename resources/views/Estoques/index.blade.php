    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Controle de Estoques') }}
            </h2>
            <a href="{{ route('estoques.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                + Novo Estoque
            </a>
        </div>
    </x-slot>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    @forelse ($estoques as $estoque)
                        <div class="flex flex-col justify-between border border-gray-200 p-5 rounded-lg hover:shadow-lg transition bg-gray-50">
                            <div>
                                <h3 class="font-bold text-xl text-gray-900 mb-2">Estoque ID: {{ $estoque->estoque_id }}</h3>
                                ...
                                <p class="text-sm text-indigo-600 font-medium flex items-center">
                                    <span class="font-semibold text-gray-800 mr-1">Capacidade:</span> {{ $estoque->capacidade }}
                                </p>
                            </div>
