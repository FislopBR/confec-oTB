                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    @forelse ($clients as $clients)
                        <div class="flex flex-col justify-between border border-gray-200 p-5 rounded-lg hover:shadow-lg transition bg-gray-50">
                            <div>
                                <h3 class="font-bold text-xl text-gray-900 mb-2">{{ $clients->nome }}</h3>
                                <p class="text-sm text-gray-600 mb-1">
                                    <span class="font-semibold text-gray-800">CPF:</span> {{ $clients->cpf }}
                                </p>
                                <p class="text-sm text-indigo-600 font-medium flex items-center">
                                    <span class="mr-1">📞</span> {{ $clients->telefone }}
                                </p>
                            </div>

                            <!-- Botões de Ação no Rodapé do Card -->
                            <div class="flex items-center justify-end mt-6 pt-4 border-t border-gray-200 space-x-4">
                                <a href="{{ route('clients.edit', $clients->id) }}" class="text-indigo-600 hover:text-indigo-900 text-sm font-semibold flex items-center">
                                    Editar
                                </a>

                                <form action="{{ route('clients.destroy', $clients->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir este cliente?')">
                                    @csrf
                                    @method('DELETE')
